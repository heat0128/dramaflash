...
export default async function DiscoverPage() {
  ...
  const categories = Array.from(new Set(
    [...(forYou || []), ...(latest || [])].map(s => s.category).filter(Boolean)
  )) as string[]
  ...
  return (
    <>
      ...
      <main className="pt-[68px] pb-[90px] min-h-screen">
        {trending?.[0] && (
          <Link href={`/series/${trending[0].id}`}
            ...
            {trending[0].cover_url && (
              <img src={trending[0].cover_url} alt=""
        ...
        {categories.length > 0 && (
          <>
            ...
            <div className="flex gap-2 px-4 overflow-x-auto no-scrollbar pb-2">
              {categories.map(cat => (
                <Link key={cat} href={`/discover/${encodeURIComponent(cat)}`}
