...
export function InstallPrompt() {
  ...
  const deferredRef = useState<any>(null)
  ...
  const install = async () => {
    const deferred = (deferredRef as any)[0]
    if (deferred) {
      deferred.prompt()
  ...
  if (isIOS) {
    return (
      <div className="fixed inset-0 z-[60] bg-black/70 backdrop-blur flex items-end justify-center"
        onClick={dismiss}>
        <div className="bg-[#1c1c1e]/98 backdrop-blur-2xl w-full max-w-[480px] rounded-t-3xl p-7 pb-10"
          ...
          <div className="bg-white/[0.06] rounded-2xl p-2">
            <Step n={1} text={<>Tap the <b>Share</b> button below in Safari</>} />
            <Step n={2} text={<>Scroll and tap <b>Add to Home Screen</b></>} />
            <Step n={3} text={<>Tap <b>Add</b> in the top-right</>} last />
          </div>
          <button onClick={dismiss}
            className="w-full mt-5 bg-brand-gradient py-3.5 rounded-xl font-bold">Got It</button>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-24px)] max-w-[456px] bg-[#141414]/95 backdrop-blur-2xl border border-white/10 rounded-2xl p-3.5 flex items-center gap-3 z-[50] shadow-2xl">
      ...
      <button onClick={install} className="bg-brand-gradient px-4 py-2 rounded-full text-xs font-bold flex-shrink-0">Install</button>
    </div>
  )
}
...
function Step({ n, text, last }: { n: number; text: React.ReactNode; last?: boolean }) {
  return (
    <div className={`flex items-center gap-3.5 p-3 ${!last ? 'border-b border-white/[0.06]' : ''}`}>
      <div className="w-6 h-6 rounded-full bg-brand-orange/20 text-brand-orange flex items-center justify-center font-extrabold text-xs flex-shrink-0">{n}</div>
      <div className="text-[13.5px] leading-snug [&_b]:text-brand-orange [&_b]:font-bold">{text}</div>
    </div>
  )
}
